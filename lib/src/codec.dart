...
BigInt decodeUint256(Iterable b) {
  return BigInt.parse(hex.encode(b.take(32).toList() as List<int>), radix: 16);
}
...
Uint8List decodeFixedBytes(Iterable b, int length) {
  return Uint8List.fromList(b.take(length).toList() as List<int>);
}

Uint8List decodeBytes(Iterable b) {
  var length = decodeInt(b);
  return Uint8List.fromList(b.skip(32).take(length).toList() as List<int>);
}
...
String decodeString(Iterable b) {
  var length = decodeInt(b);
  return String.fromCharCodes(b.skip(32).take(length) as Iterable<int>);
}
...
String decodeAddress(Iterable b) {
  return hex.encode(b.take(32).skip(12).toList() as List<int>);
}

Uint8List encodeAddress(String a) {
  Uint8List encoded = hex.decode(a) as Uint8List;
